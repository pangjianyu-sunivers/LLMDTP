...
def generate_response(prompt,prev_q,prev_flag):
    if st.session_state.option == 'ChatZOC':
        url = 'http://127.0.0.1:8892/chat_zh'
    else:
        url = 'http://127.0.0.1:8892/chat'
...
def show_point(points,full_point):
    with st.sidebar:  
        ############## 确认选择
        if st.button('结束并查询分数'):       
            st.session_state.chat = 'off'
            
    if st.session_state.chat == 'off':
        st.write('问答结束')
        st.write('您的分数为:',round(points/full_point*100,2))
        time2 = time.time()
        exam_time = time2 - st.session_state.time
        st.write('您的用时为:',round(exam_time),'s')
        st.session_state.point = 0
        st.session_state.state_all = 0
        st.write('请点击左边栏返回标准病人选择界面重新选择')
...
if st.session_state.chat == 'on':
    ...
    if st.session_state.prompt:
        ...
        if audio_data:
            ...
            print('无重复')
            try:
                tts = gTTS(output, lang='zh', tld='com')
    ...
    prompt_input = st.chat_input("打字沟通或使用键盘自带的语音输入")
    if prompt_input:
