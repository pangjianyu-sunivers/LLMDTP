...
emb_fn = embedding_functions.SentenceTransformerEmbeddingFunction(model_name="sentence-transformers/distiluse-base-multilingual-cased-v1")
...
def get_random_response_not_related():
    return send_to_llm('请生成一句话表示不想回答')
def get_random_response_first():
    return send_to_llm('请生成一句话表达对于问题的不解，需要再问一遍')
def get_random_response_second():
    return send_to_llm('请生成一句话表达对于问题的不解')
