We offer two trial options for LLMDTP for you to choose from:

#### Method 1:
Directly open your browser and access the link http://chatzoc.xyz:8501. The password is:
chatzocPW!@#
The model and framework used in this link are exactly the same as those we use in our RCT.
...
#### Method 2:
# In the Code ocean, we only need to run the following code in a terminal to open the LLMDTP server, :
cd to_NM
nohup python llm_sample_api.py &
cd demo_NM
nohup python teaching_api.py &
streamlit run SP_choose.py
